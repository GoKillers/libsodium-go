...
func TestRandomBytesBufDeterministic(t *testing.T) {
	...
	for i := 0; i < 10000; i++ {
		seed := RandomBytes(RandomBytesSeedBytes())

		// Give a & b the same random length.
		f.Fuzz(&a)
		b = a

		RandomBytesBufDeterministic(a, seed)
		RandomBytesStir()
		RandomBytesBufDeterministic(b, seed)
...
func TestRandomBytesSetImplementation(t *testing.T) {
	implementations := make(map[string](*_Ctype_struct_randombytes_implementation))
	implementations["salsa20"] = RandomBytesSalsa20Implementation
	implementations["sysrandom"] = RandomBytesSysRandomImplementation

	// Loop through all implementations
	for n, i := range implementations {
		RandomBytesSetImplementation(i)
		if RandomBytesImplementationName() != n {
			t.Errorf(
				"Incorrect implementation %v should be %v",
				RandomBytesImplementationName(),
				n,
			)
		}
